    ...
    def _evaluate(self, Gs, Gs_kwargs, num_gpus):
        ...
        if os.path.isfile(cache_file):
            ...
            for idx, images in enumerate(self._iterate_reals(minibatch_size=minibatch_size)):
                ...
                if images[:end-begin].shape[1] != 3:
                    images_rgb = np.repeat(images[:end-begin], 3, axis=1)
                else:
                    images_rgb    = images[:end-begin]
                #activations[begin:end] = inception.run(images[:end-begin], num_gpus=num_gpus, assume_frozen=True)
                activations[begin:end] = inception.run(images_rgb, num_gpus=num_gpus, assume_frozen=True)
        ...
        for gpu_idx in range(num_gpus):
            with tf.device('/gpu:%d' % gpu_idx):
                ...
                if images.shape[1] != 3:
                    images = tf.transpose(images, [0,2,3,1]) # NxWxHxC
