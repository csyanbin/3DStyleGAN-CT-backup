...
#         with tf.control_dependencies( [print_op] ):  # ignore surrounding control_dependencies
...
def lerp_clip(a: TfExpressionEx, b: TfExpressionEx, t: TfExpressionEx) -> TfExpression:
    ...
    with tf.name_scope("LerpClip"):
        ...
        res = a + (b - a) * tf.clip_by_value(t, 0.0, 1.0)
        res = tf.cast( res, tf.float32 )
        return res
...
def image_resize(images, img_shape=[ 256, 256 ] ):
    """Convert a minibatch of images from uint8 to float32 with configurable dynamic range.
    Can be used as an input transformation for Network.run().
    """
    images = tf.image.resize( images, img_shape )
    return images 
...
# 3D Images 
def convert_3d_images_from_uint8(images, drange=[-1,1], nhwdc_to_nchwd=False):
    """Convert a minibatch of images from uint8 to float32 with configurable dynamic range.
    Can be used as an input transformation for Network.run().
    """
    images = tf.cast(images, tf.float32)
...
def convert_3d_images_to_float(images, drange=[-1,1], nchwd_to_nhwdc=False, shrink=1):
    """Convert a minibatch of images from float32 to uint8 with configurable dynamic range.
    Can be used as an output transformation for Network.run().
    """
    images = tf.cast(images, tf.float32)
    ...
    images = ( images - drange[ 0 ] ) * scale + 0.0          # [0.0, 1.0]
    return images
