...
dtypeGlob = 'float32' # for config-f, as it runs out of memory even with 16GB GPUs
structureGlob = 'auto'
...
def G_synthesis_stylegan2(
    ...
    fmap_base           = 4 << 9,     # Overall multiplier for the number of feature maps.
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    for layer_idx in range(num_layers - 1):
        ...
        shape = [1, 1, base_size[ 0 ] * 2**( res - 2), base_size[ 1 ] * 2**( res - 2 ) ]
    ...
    with tf.variable_scope('{}x{}'.format( base_size[ 0 ], base_size[ 1 ] ) ):
        with tf.variable_scope('Const'):
            x = tf.get_variable('const', shape=[1, nf(1), base_size[ 0 ], base_size[ 1 ] ], initializer=tf.initializers.random_normal())
    ...
    for res in range(3, resolution_log2 + 1):
        with tf.variable_scope('%dx%d' % (base_size[0] * 2**( res - 2), base_size[1]* 2**( res-2) )):
            x = block(x, res)
            if architecture == 'skip':
                y = upsample(y)
...
def D_stylegan2(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    images_in.set_shape([None, num_channels, int( resolution_pow2 / 4 * base_size[ 0 ] ), int( resolution_pow2 / 4 * base_size[1 ] )  ] )
    labels_in.set_shape([None, label_size])
