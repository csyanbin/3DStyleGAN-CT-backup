...
def load_dicom_convert(folder, vmin=-250, vmax=650):
    files = glob.glob(os.path.join(folder, '*.dcm'))
    files_sorted = sorted(files)
    images3d = np.zeros((512, 512, len(files)))  # 512x512xN
...
#----------------------------------------------------------------------------
def create_from_images3dCT(tfrecord_dir, image_dir, shuffle, base_size, vmin=-250, vmax=650):
    print('Loading images from "%s"' % image_dir)
    image_folders = sorted(glob.glob(os.path.join(image_dir, '*')))
    if len(image_folders) == 0:
        error('No input images found')
    else:
        print("Total: ", len(image_folders), " folders!")
    ...
    for folder in image_folders:
        ...
        if len(dcms)<=32*base_size[-1]: # <64 slices
            total_images += 1
        else:
        ...
        for folder in image_folders:
            ...
            if img.shape[-1] >= 32*base_size[-1]:
                for i in range(img.shape[-1]-32*base_size[-1]+1):
                    img_tf = img[np.newaxis, :, :, i:i+32*base_size[-1]] # 1x512x512x64
                    tfr.add_image3d_base(img_tf, base_size)
            else: # pad with 0s
                pad_img = np.zeros((512, 512, 32*base_size[-1]-img.shape[-1]))
                img_tf = np.concatenate((img, pad_img), axis=-1) # 512x512x64
                img_tf = np.expand_dims(img_tf, axis=0) # 1x512x512x64
                tfr.add_image3d_base(img_tf, base_size)
          
def load_dicom_convert256(folder, vmin=-250, vmax=650):
    files = glob.glob(os.path.join(folder, '*.dcm'))
    files_sorted = sorted(files)
    images3d = np.zeros((256, 256, len(files)))  # 256x256xN
    for i, file in enumerate(files_sorted):
        ds = dcmread(file)
        image = ds.pixel_array
        image = image * ds.RescaleSlope + ds.RescaleIntercept
...
def execute_cmdline(argv):
    ...
    p = add_command(    'create_from_images3dCT', 'Create dataset from a directory full of 3D CTCA dicom images.',
                                            'create_from_images3dCT datasets/mydataset myimagedir')

    p.add_argument(     'tfrecord_dir',     help='New dataset directory to be created')
    p.add_argument(     'image_dir',        help='Directory containing the images')
    p.add_argument(     '--shuffle',        help='Randomize image order (default: 1)', type=int, default=1)
    p.add_argument(     '--base_size',      help='Base Layer Size (e.g. 4 4 4 for 2^x sized images. 5 6 7 for 160x192x224 (5x32, 6x32, 7x32) or 64x512x512 (2x32, 16x32, 16x32) images ). (default: 2 16 16 for 64x512x512 images', nargs=3, type=int, default=[16, 16, 2] )
    ...
    p = add_command(    'create_from_images3dCT256', 'Create dataset from a directory full of 3D CTCA dicom images.',
                                            'create_from_images3dCT datasets/mydataset myimagedir')
